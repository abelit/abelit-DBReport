...
def sendMail(mail_host, mail_user, mail_pass, mail_sender, receiver, subject, content, *attachmentFilePaths): 
    mail_host = mail_host
    mail_user = mail_user
    mail_pass = mail_pass
    ...
    msg = MIMEMultipart('related')  # 发送附件时需调用 MIMEMultipart类，创建 MIMEMultipart,并添加信息头
...
域的首行左侧不能有空白字符，比如空格或者制表符，占用多行的域其后续行则必须以空白字符开头。域的信息内容中还可以包含属性，属性之间以“;”分隔，属性的格式如下：{属性名称}=”{属性值}”。    
    ...
    try:
        mailServer = smtplib.SMTP(mail_host)  # 连接腾讯邮件的服务器；SMTP（Simple Mail Transfer Protocol）即简单邮件传输协议，用于由源地址到目的地址传送邮件的规则，由它来控制信件的中转方式
        ...
        mailServer.login(mail_user, mail_pass)  # 登录邮箱
        mailServer.sendmail(mail_user, mail_receiver, msg.as_string())  # 发送邮件（发件人，收件人，发送内容）
        mailServer.close()  # 关闭邮件发送服务
        print('Sent email to %s successfully' % mail_receiver)
...
if __name__ == '__main__':
    ...
    rptsettings  = settings.ReportSetting().getReportSetting()
    
    sendMail(rptsettings['mail_host'],rptsettings['mail_user'],rptsettings['mail_pass'],rptsettings['mail_sender'],rptsettings['mail_receiver'], rptsettings['mail_subject'], rptsettings['mail_content'], rptsettings['report_filename'])
